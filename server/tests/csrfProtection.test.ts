...
describe("U4 CSRF protection", () => {
  ...
  ])("allows an exact-origin MFA challenge for a pending-MFA session: %s", (path) => {
    const next = jest.fn();
    csrfProtection(
      request({
        path,
        session: { pendingMfaUserId: "founder-user" },
        headers: { origin: "https://app.myperfectmeals.ai" },
      }),
      response(),
      next,
  ...
  it("still rejects a pending-MFA challenge from an untrusted origin", () => {
    const res = response();
    const next = jest.fn();
    csrfProtection(
      request({
