...
router.post("/redeem/:token", requireAuth, async (req, res) => {
  ...
  try {
    ...
    const existing = await db.execute(sql`
      ...
      WHERE promotion_id = ${promo.id} AND redeemed_by_user_id = ${userId}
      LIMIT 1
    `);
    ...
    if (promo.type === "extended_trial") {
      // Extend trial: max(current trialEndsAt, now()) + trialDays
      ...
      // Return the Stripe promo code — client will apply at checkout
