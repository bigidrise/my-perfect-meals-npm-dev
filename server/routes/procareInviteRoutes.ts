...
import { getInviteMetadata, acceptInviteByToken } from "../services/procareInviteService";
...
router.post("/token/:token/accept", requireAuth, async (req, res) => {
  try {
      ...
      switch (error.code) {
        ...
        case "EMAIL_MISMATCH":
          return res.status(403).json({
            ...
            message: `This invitation was sent to ${error.maskedEmail}. Please sign in with that account to continue.`,
    ...
    res.json({ success: true, membership: result.result });
  } catch (err) {
