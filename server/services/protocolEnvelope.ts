...
When possible, incorporate: ${prefList}.`;
  ...
  if (envelope.cuisinePreference) {
    ...
    const intensityDepth: Record<string, string> = {
      light: "Apply full cultural identity with lighter preparation. IMPORTANT: 'Light' means lighter cooking method and caloric density — NOT a reduction in cultural authenticity or dish format. The dish must still look, taste, and be named like it belongs to this cuisine. Cultural dish format, naming, and preparation style remain authentic — only the cooking method is lightened (e.g., baked instead of fried, braised with less fat, grilled instead of sautéed). Do NOT fall back to generic Western formats (salad, bowl, wrap) — these are off-limits even in Light mode. A light soul food meal is still a soul food meal: braised collard greens with less smoked meat, grilled chicken thighs instead of fried — not a 'Chicken and Greens Salad.' A light Ethiopian meal is still an Ethiopian meal: less niter kibbeh, not a grain bowl.",
