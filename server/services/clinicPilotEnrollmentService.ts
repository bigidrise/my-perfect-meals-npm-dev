...
import { clinicPilotEnrollmentLinks, clinicTrialEntitlements, organizationalPilotEvents, organizationalPilotParticipants, organizationalPilots, professionalTemporaryAccessEntitlements } from "../db/schema/pilotProgram";

const hashToken = (token: string) => crypto.createHash("sha256").update(token).digest("hex");
const newToken = () => crypto.randomBytes(32).toString("hex");
export const APPROVED_TEMPORARY_ACCESS_DURATIONS = [7, 14, 30] as const;
export function assertTemporaryAccessDuration(value: number): asserts value is 7 | 14 | 30 {
  if (!APPROVED_TEMPORARY_ACCESS_DURATIONS.includes(value as 7 | 14 | 30)) {
    throw new Error("INVALID_ACCESS_DURATION");
  }
}
export function clinicTrialEnd(start: Date, durationDays = 30): Date {
  assertTemporaryAccessDuration(durationDays);
  return new Date(start.getTime() + durationDays * 24 * 60 * 60 * 1000);
}

export async function createClinicPilotEnrollmentLink(input: { businessId: string; pilotId: string; actorUserId: string; capacity?: number; expiresAt?: Date | null; accessDurationDays?: number }) {
...
export async function enrollClinicPatientInTransaction(tx: any, rawToken: string, userId: string) {
  ...
  return (async () => {
    ...
    const startsAt = now;
    const endsAt = clinicTrialEnd(startsAt, link.accessDurationDays);
...
export async function getActiveProfessionalTemporaryAccess(userId: string, now = new Date()) {
  const [row] = await db.select().from(professionalTemporaryAccessEntitlements).where(and(
    eq(professionalTemporaryAccessEntitlements.userId, userId),
    eq(professionalTemporaryAccessEntitlements.status, "active"),
    lte(professionalTemporaryAccessEntitlements.startsAt, now),
    gt(professionalTemporaryAccessEntitlements.endsAt, now),
  )).limit(1);
  return row ?? null;
}