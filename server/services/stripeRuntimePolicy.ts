...
export function getStripeKeyMode(key = getStripeSecretKey()): StripeKeyMode {
...
export function canOwnStripeBilling(key = getStripeSecretKey()): boolean {
  const mode = getStripeKeyMode(key);
  return mode !== "LIVE" || isProductionBillingRuntime();
}

export function assertStripeBillingOwnership(key = getStripeSecretKey()): void {
