...
export async function runMyPerfectMenuMigration(database: MigrationDatabase) {
  await database.transaction(async (transaction) => {
    ...
    await transaction.execute(sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS my_perfect_menu_preferences jsonb`);
    await transaction.execute(sql`ALTER TABLE household_profiles ADD COLUMN IF NOT EXISTS my_perfect_menu_preferences jsonb`);
  });
}