...
export async function runOneTouchMigration(database: MigrationDatabase): Promise<void> {
  await database.transaction(async (transaction) => {
    ...
    await transaction.execute(sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS one_touch_history jsonb`);
  });
}