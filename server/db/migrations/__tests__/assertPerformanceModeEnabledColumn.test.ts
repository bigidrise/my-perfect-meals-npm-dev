/**
 * server/db/migrations/__tests__/assertPerformanceModeEnabledColumn.test.ts
 *
 * Unit tests for the performance_mode_enabled startup guard.
 *
 * Verifies that assertPerformanceModeEnabledColumn() throws a descriptive
 * error (containing "STARTUP GUARD" and "performance_mode_enabled") when the
 ...
 * Run: npx tsx server/db/migrations/__tests__/assertPerformanceModeEnabledColumn.test.ts
 */
...
async function runTests() {
  ...
  try {
    await assertPerformanceModeEnabledColumn(fakeDb([]) as any);
    ...
    assert(
      err.message.includes("performance_mode_enabled"),
      'error message contains "performance_mode_enabled"',
    );
  }
  ...
  try {
    await assertPerformanceModeEnabledColumn(fakeDbDirect([]) as any);
  ...
  try {
    await assertPerformanceModeEnabledColumn(
      fakeDb([{ column_name: "performance_mode_enabled" }]) as any,
    );
  ...
  try {
    await assertPerformanceModeEnabledColumn(
      fakeDbDirect([{ column_name: "performance_mode_enabled" }]) as any,
    );
  ...
  try {
    ...
    await assertPerformanceModeEnabledColumn(brokenDb as any);
