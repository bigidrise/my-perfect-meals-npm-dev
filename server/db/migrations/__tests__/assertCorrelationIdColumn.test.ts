 ...
 *
 * Unit tests for the Safety PIN startup guard.
 *
 * Verifies that assertCorrelationIdColumn() throws a descriptive error
 * (containing "STARTUP GUARD" and "correlation_id") when the DB query
...
async function runTests() {
  ...
  try {
    ...
    assert(
      err.message.includes("correlation_id"),
      'error message contains "correlation_id"',
    );
  }
  ...
  try {
    await assertCorrelationIdColumn(
      fakeDb([{ column_name: "correlation_id" }]) as any,
    );
  ...
  try {
    await assertCorrelationIdColumn(
      fakeDbDirect([{ column_name: "correlation_id" }]) as any,
    );
  ...
  try {
    // Simulate a broken response where neither .rows nor the result is an array
    const brokenDb = {
