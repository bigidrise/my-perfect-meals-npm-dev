...
export default function RestaurantGuideCard({ className = "" }: RestaurantGuideCardProps) {
  return (
    <div className={`p-4 bg-gradient-to-br from-black/60 via-orange-600 to-black/80 rounded-lg text-white ${className}`}>
      ...
      <div className="text-xs bg-amber-100 text-gray-800 px-2 py-1 rounded">
        {isIosNativeShell() ? "🔒 Unlock with Pro" : "🔒 Unlock with Pro Plan – $29.99/month"}
      </div>
    </div>
  );
}