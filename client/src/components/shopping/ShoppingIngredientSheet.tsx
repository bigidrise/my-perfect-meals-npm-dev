...
const COMPANION_VERDICT_CONFIG = {
  buy:     { icon: "🟢", label: "Recommended",     bg: "bg-emerald-500/15 border-emerald-500/40", color: "text-emerald-400", chipBg: "bg-emerald-500/20 border-emerald-500/40" },
  caution: { icon: "🟡", label: "Use Caution",     bg: "bg-amber-500/15 border-amber-500/40",  color: "text-amber-400",  chipBg: "bg-amber-500/20 border-amber-500/40"  },
  skip:    { icon: "🔴", label: "Not Recommended", bg: "bg-rose-500/15 border-rose-500/40",    color: "text-rose-400",   chipBg: "bg-rose-500/20 border-rose-500/40"    },
};
...
function CompanionLayout({ result, companionName, onClose, onSaveForReview }: {
  result: IngredientScanResult;
  companionName: string;
  onClose: () => void;
  onSaveForReview?: () => void;
}) {
  const verdictCfg = COMPANION_VERDICT_CONFIG[result.verdictLevel ?? "caution"];
  const considerations = result.mayNotAlignWith ?? [];
  const betterOptions = result.betterFor ?? [];
    ...
    <div className="px-4 pt-10 pb-8 max-w-lg mx-auto">
      ...
      <div className="flex items-center gap-3 mb-5">
        ...
        <div className="flex-1">
          <p className="text-xs text-orange-400 font-bold uppercase tracking-wide">Companion Smart Scan</p>
      ...
      {/* Big Verdict Pill */}
      <div className={`rounded-2xl border p-5 mb-5 text-center ${verdictCfg.bg}`}>
        <div className="text-4xl mb-2">{verdictCfg.icon}</div>
      ...
      <div className="rounded-xl bg-white/5 border border-white/10 p-4 mb-4">
        <p className="text-xs font-bold uppercase tracking-wide text-white/40 mb-3">
          🐾 Why This Matters For {companionName}
        </p>
        <p className="text-sm text-white/85 leading-relaxed">{result.overallSummary}</p>
      </div>

      {/* Active Considerations chips */}
      {considerations.length > 0 && (
        <div className="mb-5">
          <p className="text-xs font-bold uppercase tracking-wide text-white/40 mb-2">Active Considerations</p>
          <div className="flex flex-wrap gap-2">
            {considerations.map((item, i) => (
              <span
                key={i}
                className="bg-amber-500/15 border border-amber-500/30 text-amber-300 text-xs font-semibold px-3 py-1.5 rounded-full"
              >
                {item}
              </span>
            ))}
      ...
      {betterOptions.length > 0 && (
        <div className="mb-5">
          <p className="text-xs font-bold uppercase tracking-wide text-white/40 mb-2">Better Options</p>
          <div className="space-y-2">
            {betterOptions.map((opt, i) => (
              <div key={i} className="flex items-start gap-2.5 rounded-lg bg-emerald-500/8 border border-emerald-500/15 px-3 py-2.5">
      ...
      <p className="text-[10px] text-white/15 text-center mt-4 leading-relaxed px-2">
        Companion wellness guidance only — not veterinary advice. Always consult your veterinarian for medical concerns.
      </p>
    </div>
  );
}
