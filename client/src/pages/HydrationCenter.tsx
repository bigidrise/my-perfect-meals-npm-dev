    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="min-h-screen bg-[radial-gradient(circle_at_top,#0c4a6e_0%,#082f49_28%,#020617_68%)] text-white">
      ...
      <main className="mx-auto max-w-5xl space-y-4 px-4 pb-28" style={{ paddingTop: "calc(env(safe-area-inset-top, 0px) + 5.75rem)" }}>
        {loading ? <div className="grid min-h-80 place-items-center"><RefreshCw className="h-7 w-7 animate-spin text-sky-300" /></div> : error ? (
          <Card className="border-white/10 bg-slate-950/45"><CardContent className="flex min-h-72 flex-col items-center justify-center text-center"><Info className="mb-3 h-8 w-8 text-amber-300" /><p className="font-semibold">Hydration is unavailable</p><p className="mt-1 text-sm text-white/55">{error}</p><Button onClick={() => void load()} className="mt-4">Try again</Button></CardContent></Card>
        ) : state && policy && copy ? <>
            ...
            <Card className="overflow-hidden border-white/10 bg-slate-950/45 shadow-2xl backdrop-blur-xl">
              <CardContent className="p-5 sm:p-6">
                <div className="flex items-start justify-between gap-3">
                  <div><p className="text-xs uppercase tracking-[.2em] text-sky-200/55">Today</p><h2 className="mt-1 text-xl font-semibold">Total fluids and plain water</h2></div>
                  <Droplets className="h-7 w-7 text-sky-300" />
                </div>
                <div className="mt-5 grid grid-cols-2 gap-3">
                  <div className="rounded-2xl border border-sky-300/15 bg-sky-400/10 p-4"><p className="text-xs text-sky-100/55">Total fluids</p><p className="mt-1 text-3xl font-bold">{mlToOz(today.totalFluidsMl)} <span className="text-sm font-medium text-white/45">oz</span></p><p className="text-xs text-white/35">{today.totalFluidsMl.toLocaleString()} mL</p></div>
                  <div className="rounded-2xl border border-white/10 bg-white/[.04] p-4"><p className="text-xs text-white/55">Plain water</p><p className="mt-1 text-3xl font-bold">{mlToOz(today.plainWaterMl)} <span className="text-sm font-medium text-white/45">oz</span></p><p className="text-xs text-white/35">{today.plainWaterMl.toLocaleString()} mL</p></div>
                </div>
                {!!today.beverageMix.length && <div className="mt-4 flex flex-wrap gap-2">{today.beverageMix.map((item) => <Badge key={item.beverageClass} variant="outline" className="border-white/10 text-white/65">{BEVERAGES.find(([value]) => value === item.beverageClass)?.[1] || item.beverageClass}: {mlToOz(item.amountMl)} oz</Badge>)}</div>}
                <div className="mt-5 rounded-xl border border-white/10 bg-black/15 p-4"><div className="flex items-center gap-2"><ShieldCheck className="h-5 w-5 text-sky-300" /><h3 className="font-semibold">{copy.title}</h3></div><p className="mt-2 text-sm leading-relaxed text-white/60">{copy.body}</p><p className="mt-2 text-xs text-white/35">Preferences may change practical suggestions, never physiological requirements.</p></div>
              </CardContent>
            </Card>

            <Card className="border-white/10 bg-slate-950/45 backdrop-blur-xl"><CardContent className="p-5">
              <div className="flex items-center gap-2"><Plus className="h-5 w-5 text-sky-300" /><h2 className="font-semibold">Log a fluid</h2></div>
              <label className="mt-4 block text-xs text-white/50">Beverage</label>
              ...
              <Button disabled={saving || Number(customAmount) <= 0} onClick={() => void addFluid(Number(customAmount), customUnit)} className="mt-2 w-full bg-sky-500 text-slate-950 hover:bg-sky-400" data-testid="hydration-add-custom">{saving ? "Saving…" : "Add to today"}</Button>
              <p className="mt-3 text-xs leading-relaxed text-white/35">Fluid contribution is tracked separately from calories, sugar, sodium, and other nutrition.</p>
            </CardContent></Card>
          </section>

          <section className="grid gap-4 lg:grid-cols-2">
            <Card className="border-white/10 bg-slate-950/45 backdrop-blur-xl"><CardContent className="p-5">
              <div className="flex items-center gap-2"><Settings2 className="h-5 w-5 text-violet-300" /><h2 className="font-semibold">What gets in the way?</h2></div>
              <p className="mt-2 text-sm text-white/50">Optional setup helps choose practical strategies. It never changes a hydration requirement.</p>
              <div className="mt-4 flex flex-wrap gap-2">{BARRIERS.map(([code, label]) => <button key={code} onClick={() => toggleBarrier(code)} className={`rounded-full border px-3 py-2 text-xs transition ${selectedBarriers.includes(code) ? "border-violet-300/50 bg-violet-400/20 text-violet-50" : "border-white/10 bg-white/[.03] text-white/55 hover:bg-white/[.07]"}`}>{selectedBarriers.includes(code) && <Check className="mr-1 inline h-3 w-3" />}{label}</button>)}</div>
              <Textarea value={barrierNote} onChange={(event) => setBarrierNote(event.target.value)} maxLength={500} placeholder="Optional note about what makes this hard" className="mt-3 border-white/10 bg-white/5 text-white placeholder:text-white/30" />
              <div className="mt-4 grid grid-cols-3 gap-2">
                {[["flavor", "Flavor", ["no_preference", "citrus", "berry", "mild"]], ["temperature", "Temperature", ["no_preference", "cold", "room", "warm"]], ["carbonation", "Bubbles", ["no_preference", "still", "sparkling"]]].map(([key, label, values]) => <label key={key as string} className="text-[11px] text-white/45">{label as string}<select value={preferences[key as string]} onChange={(event) => setPreferences((current) => ({ ...current, [key as string]: event.target.value }))} className="mt-1 w-full rounded-md border border-white/10 bg-slate-900 px-2 py-2 text-xs text-white">{(values as string[]).map((value) => <option key={value} value={value}>{value.replaceAll("_", " ")}</option>)}</select></label>)}
              </div>
              <label className="mt-4 flex items-start gap-2 rounded-xl border border-white/10 bg-white/[.03] p-3 text-xs text-white/60"><input type="checkbox" checked={consented} onChange={(event) => setConsented(event.target.checked)} className="mt-0.5" /><span>I consent to saving these optional preferences and barriers for Hydration Hub suggestions.</span></label>
            ...
            <Card className="border-white/10 bg-slate-950/45 backdrop-blur-xl"><CardContent className="p-5">
              <div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2"><Lightbulb className="h-5 w-5 text-amber-300" /><h2 className="font-semibold">Help Me Get It In</h2></div><Button size="sm" disabled={saving} onClick={() => void getHelp()} className="bg-amber-400 text-slate-950 hover:bg-amber-300"><Sparkles className="mr-1.5 h-4 w-4" />Get options</Button></div>
              <p className="mt-2 text-sm text-white/50">Small, nonnumeric strategies based on the barrier you chose.</p>
              <div className="mt-4 space-y-2">{options.length ? options.map((option) => <div key={option.id} className="rounded-xl border border-white/10 bg-white/[.04] p-3"><div className="flex items-start justify-between gap-3"><div><Badge variant="outline" className="mb-2 border-white/10 text-[10px] text-white/45">{BARRIER_LABELS[option.barrierCode] || option.barrierCode}</Badge><h3 className="text-sm font-semibold">{option.title}</h3><p className="mt-1 text-xs leading-relaxed text-white/50">{option.description}</p></div><Button size="sm" onClick={() => void chooseIntervention(option)} className="shrink-0 bg-white/10 text-white hover:bg-white/20">{option.destinationType === "beverage_creator" ? "Create" : "Try it"}</Button></div></div>) : <div className="rounded-xl border border-dashed border-white/10 p-6 text-center text-sm text-white/40">Save a barrier, then ask for practical options.</div>}</div>
            </CardContent></Card>
          </section>

          <section className="grid gap-4 md:grid-cols-[1.15fr_.85fr]">
            <Card className="border-white/10 bg-slate-950/45 backdrop-blur-xl"><CardContent className="p-5">
              <div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-2"><Clock3 className="h-5 w-5 text-sky-300" /><h2 className="font-semibold">Descriptive history</h2></div><div className="flex rounded-lg border border-white/10 bg-black/15 p-1">{(["today", "7", "30"] as const).map((window) => <button key={window} onClick={() => setHistoryWindow(window)} className={`rounded-md px-3 py-1.5 text-xs ${historyWindow === window ? "bg-sky-400/20 text-sky-50" : "text-white/40"}`}>{window === "today" ? "Today" : `${window} days`}</button>)}</div></div>
              {historyWindow === "today" ? <div className="mt-4 space-y-2">{state.todayHistory?.length ? state.todayHistory.map((item) => <div key={item.id} className="flex items-center justify-between rounded-xl border border-white/8 bg-white/[.04] px-3 py-2.5"><div><span className="text-sm text-white/65">{new Intl.DateTimeFormat(undefined, { hour: "numeric", minute: "2-digit" }).format(new Date(item.intakeTime))}</span><span className="ml-2 text-xs text-white/35">{BEVERAGES.find(([value]) => value === item.beverageClass)?.[1] || item.beverageClass}</span></div><span className="font-medium">{mlToOz(item.amountMl)} oz <span className="ml-1 text-xs font-normal text-white/35">{item.amountMl} mL</span></span></div>) : <p className="rounded-xl border border-dashed border-white/10 p-5 text-center text-sm text-white/45">No fluids logged yet today.</p>}</div> : <div className="mt-4 space-y-2">{dailyRows.length ? dailyRows.map((row) => <div key={row.localDate} className="grid grid-cols-[1fr_auto_auto] gap-4 rounded-xl border border-white/8 bg-white/[.04] px-3 py-2.5 text-sm"><span className="text-white/60">{new Date(`${row.localDate}T12:00:00`).toLocaleDateString(undefined, { month: "short", day: "numeric" })}</span><span>{mlToOz(row.totalMl)} oz total</span><span className="text-white/40">{mlToOz(row.plainWaterMl)} oz water</span></div>) : <p className="rounded-xl border border-dashed border-white/10 p-5 text-center text-sm text-white/45">No entries in this window.</p>}</div>}
              <p className="mt-3 text-xs text-white/35">History describes what you logged. It does not diagnose hydration status or prescribe a target.</p>
            </CardContent></Card>

            <Card className="border-white/10 bg-slate-950/45 backdrop-blur-xl"><CardContent className="p-5">
              <div className="flex items-center gap-2"><BookOpen className="h-5 w-5 text-sky-300" /><h2 className="font-semibold">Learn & reflect</h2></div>
              <p className="mt-3 text-sm leading-relaxed text-white/55">Reviewed sources explain hydration concepts without converting population references into a personal target.</p>
              <div className="mt-4 grid grid-cols-2 gap-2"><div className="rounded-xl border border-white/10 bg-white/[.04] p-3"><p className="text-xs text-white/45">Options tried</p><p className="mt-1 text-2xl font-semibold">{state.outcomeCounts?.accepted || 0}</p></div><div className="rounded-xl border border-white/10 bg-white/[.04] p-3"><p className="text-xs text-white/45">Completed</p><p className="mt-1 text-2xl font-semibold">{state.outcomeCounts?.completed || 0}</p></div></div>
              ...
              <p className="mt-4 text-xs leading-relaxed text-white/35">Products, sponsorships, and affiliate relationships never determine nutrition or clinical eligibility. Hydration Hub Phase 1 does not recommend products or supplements.</p>
            </CardContent></Card>
          </section>
        </> : null}
      </main>
