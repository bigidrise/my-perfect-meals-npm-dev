...
export default function RestaurantGuidePage() {
  ...
  const generateMealsMutation = useMutation({
    mutationFn: async (params: { restaurantName: string; craving: string; cuisine: string; zipCode: string }) => {
    ...
    onSuccess: (data) => {
      ...
      saveRestaurantCache({
        restaurantData: data,
        restaurant: data.restaurantInfo?.name || restaurantInput,
        craving: cravingInput,
        cuisine: matchedCuisine || "",
        generatedAtISO: new Date().toISOString(),
      });
        ...
        <Card data-testid="restaurant-guide-form" className="bg-black/10 backdrop-blur-lg border border-white/20 shadow-xl rounded-2xl">
          ...
          <CardContent className="p-6">
            ...
            <div className="space-y-3 mb-6">
              ...
              <div>
                <label htmlFor="zip-input" className="block text-sm font-semibold text-white mb-2">
                  Your ZIP Code
                </label>
                <div className="relative">
                  <Input
                    data-testid="restaurant-guide-zip"
                    id="zip-input"
              ...
              <div data-testid="restaurant-guide-results" data-wt="rg-results-list" className="space-y-6 mb-6">
                <div className="mb-4">
                  <h2 className="text-xl font-bold text-white">
                    🍽️ Recommended Meals at {restaurantInfo?.name || restaurantInput.split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join(' ')}
                  </h2>
