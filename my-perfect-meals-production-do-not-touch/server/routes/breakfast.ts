...
// Schema for breakfast meal acceptance
const BreakfastAcceptSchema = z.object({
  meal: z.object({
    id: z.string(),
    slug: z.string(),
    name: z.string(),
  ...
  source: z.string().optional().default("breakfast")
});

// POST /api/breakfast/accept - Accept a breakfast meal template
router.post("/accept", async (req, res) => {
  try {
    const parsed = BreakfastAcceptSchema.safeParse(req.body);
    if (!parsed.success) {
      return res.status(400).json({
        error: "Invalid breakfast meal data",
    ...
    console.log(`✅ Breakfast meal accepted: ${meal.name} - ${meal.template.name} (${servings} servings)`);
    ...
    console.error("Error accepting breakfast meal:", error);
    res.status(500).json({
      error: "Failed to save breakfast meal",
