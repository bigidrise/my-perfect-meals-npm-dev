...
// Schema for lunch meal acceptance
const LunchAcceptSchema = z.object({
  meal: z.object({
    id: z.string(),
    slug: z.string(),
    name: z.string(),
  ...
  source: z.string().optional().default("lunch")
});

// POST /api/lunch/accept - Accept a lunch meal template
router.post("/accept", async (req, res) => {
  try {
    const parsed = LunchAcceptSchema.safeParse(req.body);
    if (!parsed.success) {
      return res.status(400).json({
        error: "Invalid lunch meal data",
    ...
    console.log(`✅ Lunch meal accepted: ${meal.name} - ${meal.template.name} (${servings} servings)`);
    ...
    console.error("Error accepting lunch meal:", error);
    res.status(500).json({
      error: "Failed to save lunch meal",
