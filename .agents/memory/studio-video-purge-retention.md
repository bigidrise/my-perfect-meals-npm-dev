...
**How to apply:** any future private media cleanup worker should use a per-item renewable lease, make the retained record immutable (or version-locked) before object deletion, delete every original/derivative first, and only then persist the terminal state with cleared references. Keep retry eligibility tied to the original expiration deadline.