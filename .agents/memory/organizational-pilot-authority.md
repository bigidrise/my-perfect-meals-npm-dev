...
The Business commercial-access window owns the shared start and end timestamps. Organizational-pilot dates are compatibility/history mirrors and must match that window exactly; they are not an independently writable clock. Normal commercial pilot access requires an active pilot, active participation, and current time inside the Business window. Preparing grants no normal commercial entitlement.

A preapproved email authorization is only signup provenance. Claiming it binds an immutable user identity but grants no paid tier, organization role, provider authority, Studio access, or affiliate authority. The configured pilot duration starts only when that claimed authorization is attached during explicit organization setup by an already-authorized canonical owner/admin.
...
Guided pilot assignments and operational communications are versioned, organization-owned records. Their progress, retries, suppression, and completion history never pause, extend, restart, or otherwise modify the Business commercial window.
...
**How to apply:** Keep authorized-program provenance and capacity separate while using Business commercial access as the sole timing authority. Establish its window once at successful setup activation; signup delay must not consume the configured duration. Extensions and termination update the organization clock atomically, while paid or independently authorized arrangements remain untouched. Any organizational-pilot mirror must be written from the same window and fail closed on disagreement. Snapshot guided-program version and assignment pack once, store progress server-side, and derive all communication cadence from the authoritative window. Admin self-service may issue and revoke unused authorizations, and authorized pilot creation must lock and consume one exact authorization. Store pending invitations without users, associate real accounts only on acceptance, and leave role, ProCare, care-team, and client relationships as independent authorities.