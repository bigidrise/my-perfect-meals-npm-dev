...
**How to apply:** Use a permanent tenant ID for organization-owned data. Treat creator, business authority, internal admin, and external manager as memberships. Require server-validated organization/location selection, and never allow removal of the last controlling authority without a replacement. Render every chooser from one authenticated server availability contract. Organization comes from active authorized organization/location pairs; Studio requires provider identity plus canonical effective Studio entitlement. Readiness only controls the post-selection destination. Choosing Organization with multiple options opens the Organization Hub.
