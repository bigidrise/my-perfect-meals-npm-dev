...
My Perfect Menu restoration is an ordinary page-loading exception: use three bouncing dots with an explicit restoring label, auto-open the returned category, and never let transient empty state appear authoritative. Replacement generation keeps the current cards visible until a complete new set succeeds.