...
**How to apply:** Resolve and fingerprint the validated subject document in Human Food Context, keep prompt projection bounded and subordinate to safety/diet/protocol/nutrition rules, and do not reuse the Performance DemandProfile field. Customer UI renders only active registry records and uses registry education fields without invented claims. Empty selection is valid. Adult saves replace the complete versioned document and refresh exact-account caches. Development may run the additive migration before readiness; Production DDL runs only through the explicit release-migration gate, while ordinary Production startup fails closed on missing columns.