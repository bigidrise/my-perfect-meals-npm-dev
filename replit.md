...
- 2026-02-28: Phase 6 — Builder Update Digest Notifications + Immediate Message Push. Coach edits board → flag set → midnight digest sends ONE push to client. Tablet messages → immediate push to recipient. New: boardDigestJob.ts (midnight cron), tabletNotify.ts (instant push). DB: board_update_pending + board_updated_at on users. Push only, no SMS.
- 2026-02-28: Phase 5 Fix Pack — Soft delete for shared messages (deleted_by_pro/deleted_by_client flags, per-side visibility), provider notes CRUD (edit/archive/permanent delete/bulk archive by age), modal restructure (ProClientFolderModal → button-only hub, separate MessagesModal + ProviderNotesModal). Client More page updated with soft delete for own messages. DB: Added archived, deleted_by_pro, deleted_by_client boolean columns to client_notes.
