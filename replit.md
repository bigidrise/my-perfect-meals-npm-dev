...
- **ProCare Portal Separation v1.0** (Jan 2026): Completely separate portal experiences for Trainers vs Physicians:
  - **Trainer Portal** (`/procare/trainer`): Dedicated space with green/lime theme, athlete-focused language ("Your Athletes"), and trainer-specific builders (General Nutrition, Performance/Competition)
  - **Physician Portal** (`/procare/physician`): Dedicated space with blue theme, clinical language ("Your Patients"), and clinical builders (Diabetic, GLP-1, Anti-Inflammatory)
  - Each portal has its own client list, client dashboard, and builder routes
  - Builders navigate back to the correct portal's client dashboard (not shared routes)
  - Route structure: `/procare/{trainer|physician}/clients/:id/{builder-name}`
  - Key files: `client/src/pages/procare/TrainerPortal.tsx`, `client/src/pages/procare/PhysicianPortal.tsx`, `client/src/pages/procare/TrainerClientDashboard.tsx`, `client/src/pages/procare/PhysicianClientDashboard.tsx`
  - localStorage `mpm_pro_portal_type` tracks which portal the professional is using
