...
- **Desktop Layout**: `client/src/layout/AppLayout.tsx` wraps content in `DesktopLayout` (sidebar + workspace, >=1024px) or `MobileLayout`. Only `app.myperfectmeals.com`, localhost, and `.replit.dev` get desktop layout; `myperfectmeals.com` and `www.` always get mobile. Full-screen routes (auth, onboarding, etc.) bypass the desktop layout. `/pricing` and `/paywall` are full-screen only for unauthenticated users — authenticated users see them inside desktop layout. Mobile bottom nav hidden on desktop via `useIsDesktop` hook in `Router.tsx`. Sidebar uses wouter `Link` for SPA navigation with active state highlighting. `DesktopLayout.tsx` renders a sectioned sidebar (HOME, MEAL INTELLIGENCE, TOOLS, ACCOUNT) with a WORKSPACES section for professionals only — "Personal Space" routes to `/dashboard`, "Professional Workspace" opens `WorkspaceChooser` modal which routes based on `user.professionalRole`. No direct sidebar links to Care Team or Pro Portal — all professional pages accessed through WorkspaceChooser gate only. Active state inferred from current route (`/care-team/*` or `/pro/*` = workspace active). `DesktopHeader.tsx` shows page title + plan badge + user avatar. `DashboardNew.tsx` uses a single render path for both desktop and mobile — mobile-only header chrome (MPM bar, ProfileSheet button) is hidden via `!isDesktop` conditionals. No separate desktop page components — layout wrapper handles the difference.
